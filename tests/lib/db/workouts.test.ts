...
describe("WorkoutsDB", () => {
  ...
  // ===========================================================================
  // startWorkout — single INSERT + SELECT + single
  // ===========================================================================
  describe("startWorkout", () => {
    it("inserts with default title 'Workout' and null routine_id when unset", async () => {
      // Freeze time so we can assert the exact started_at ISO string — this
      // kills the `new Date().toISOString()` path.
      ...
      try {
        ...
        mockSupabaseClient.setMockResponse(returned);

        const result = await workoutsDB.startWorkout(USER_ID, {});

        expect(result).toEqual(returned);

        // Assert the exact insert payload — catches StringLiteral mutants on
        // "Workout" / "in_progress", LogicalOperator mutants on `?? null`, and
        // StringLiteral mutants on table name.
    ...
    it("uses provided title and routine_id when passed", async () => {
      const fixedNow = new Date("2026-04-17T10:00:00.000Z");
      vi.useFakeTimers();
      vi.setSystemTime(fixedNow);
      try {
        ...
        expect(result.routine_id).toBe("routine-5");
    ...
    it("throws 'You already have an active workout' with code 23505 and logs a warning on duplicate", async () => {
      mockSupabaseClient.setMockResponse(null, {
        code: "23505",
        message: "duplicate",
      });
      ...
      try {
        await workoutsDB.startWorkout(USER_ID, {});
      } catch (err) {
        caught = err as Error & { code?: string };
      }

      expect(caught).not.toBeNull();
      expect(caught!.message).toBe("You already have an active workout");
      expect(caught!.code).toBe("23505");

      // Exact warn call — kills StringLiteral + ObjectLiteral mutants.
      expect(log.warn).toHaveBeenCalledTimes(1);
      expect(log.warn).toHaveBeenCalledWith(
        "Attempted to start workout while one is active",
        { userId: USER_ID },
      );
      // error branch is NOT hit on 23505
      expect(log.error).not.toHaveBeenCalled();
    });

    it("logs error and rethrows original error for non-23505 errors", async () => {
      const dbError = { code: "42P01", message: "table not found" };
      mockSupabaseClient.setMockResponse(null, dbError);

      await expect(workoutsDB.startWorkout(USER_ID, {})).rejects.toEqual(
        dbError,
      );
      ...
      // warn branch is NOT hit on non-23505 errors
