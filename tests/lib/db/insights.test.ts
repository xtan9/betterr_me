...
function generateLogs(
  ...
  const logs: Array<{ habit_id: string; logged_date: string; completed: boolean }> = [];
...
describe('InsightsDB', () => {
  describe('getWeeklyInsights', () => {
    ...
    it('does not return best_habit when rate is below 80%', async () => {
      ...
      const logs = [
        { habit_id: 'h1', logged_date: getLocalDateString(prevWeekStart), completed: true },
        { habit_id: 'h1', logged_date: getLocalDateString(new Date(prevWeekStart.getTime() + 86400000)), completed: true },
        { habit_id: 'h1', logged_date: getLocalDateString(new Date(prevWeekStart.getTime() + 2 * 86400000)), completed: true },
      ];
    ...
    it('returns improvement insight when rate improves >10%', async () => {
      ...
      const prevWeekLogs = generateLogs('h1', prevWeekStart, prevWeekEnd).slice(0, 5);
      // Two weeks ago: 3/7 days (~43%)
      const twoWeeksAgoLogs = generateLogs('h1', twoWeeksAgoStart, twoWeeksAgoEnd).slice(0, 3);
    ...
    it('returns decline insight when rate drops >15%', async () => {
      ...
      const prevWeekLogs = generateLogs('h1', prevWeekStart, prevWeekEnd).slice(0, 2);
      // Two weeks ago: 6/7 days (~86%)
      const twoWeeksAgoLogs = generateLogs('h1', twoWeeksAgoStart, twoWeeksAgoEnd).slice(0, 6);
      ...
      expect(decline!.params.percent).toBeLessThan(decline!.params.lastPercent as number);
    });
    ...
    it('sorts insights by priority descending', async () => {
      ...
      const prevWeekLogs = generateLogs('h1', prevWeekStart, prevWeekEnd).slice(0, 6);
