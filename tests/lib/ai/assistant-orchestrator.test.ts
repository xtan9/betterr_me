...
describe('planning discovery and draft contract',()=>{
 it.each([{questions:[]},{questions:[{dimension:'workBoundaries',question:'The requested times overlap protected commitments. Which different times should I use?'}]}])('surfaces a newly conflicting confirmed fact instead of silently continuing a draft ($questions)',({questions})=>{
  const prior:PlanningState={status:'drafted',horizon:{startDate:'2026-09-21',endDate:'2026-10-04',timezone:context.timezone},readiness:{horizon:'known',workBoundaries:'known'},facts:{workBoundaries:'At-home focus September 21 and 28, 11:00–11:30.'},assumptions:[]};
  const candidate={...output,message:'The requested 09:00–09:30 times conflict with protected commitments. Nothing has changed.',planning:{...output.planning,facts:[{dimension:'workBoundaries',state:'partial',detail:'Requested 09:00–09:30 conflicts with protected 09:00–10:00.'}],questions,draft:null}};
