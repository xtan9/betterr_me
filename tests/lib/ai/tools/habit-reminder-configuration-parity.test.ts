...
describe("AI and HTTP Habit Reminder Configuration parity", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("maps the same Habit intent through both adapters", async () => {
    ...
    expect(mockConfigureHabitReminders).toHaveBeenCalledTimes(2);
    expect(mockConfigureHabitReminders.mock.calls[0][0]).toEqual(
      mockConfigureHabitReminders.mock.calls[1][0],
    );
    expect(mockConfigureHabitReminders.mock.calls[0][0]).toEqual({
      userId: "user-123",
  ...
  it("maps typed conflicts consistently", async () => {
    mockConfigureHabitReminders.mockResolvedValue({
      type: "conflict",
      resource: "reminder",
      reason: "Habit reminder configuration conflicted",
    });
    ...
    ).resolves.toEqual({ error: "Habit reminder configuration conflicted" });
    ...
    await expect(response.json()).resolves.toEqual({
      error: "Habit reminder configuration conflicted",
    });
  });

  it("keeps unexpected persistence failures exceptional for both adapters", async () => {
    const failure = new Error("habit storage unavailable");
    mockConfigureHabitReminders.mockRejectedValue(failure);
