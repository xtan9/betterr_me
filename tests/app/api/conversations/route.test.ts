...
describe('POST /api/conversations', () => {
  ...
  it('creates conversation with model when model is provided in body', async () => {
    const mockConversation = { id: 'c1', user_id: 'user-123', title: null, model: 'gpt-5.3-codex-spark', created_at: '2026-01-01', updated_at: '2026-01-01' };
    mockCreateConversation.mockResolvedValue(mockConversation);

    const response = await POST(makeRequest('POST', { model: 'gpt-5.3-codex-spark' }));
    const data = await response.json();

    expect(response.status).toBe(201);
    expect(data.conversation).toEqual(mockConversation);
    expect(mockCreateConversation).toHaveBeenCalledWith({ user_id: 'user-123', model: 'gpt-5.3-codex-spark' });
  });
