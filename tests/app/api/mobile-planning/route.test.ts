...
it.each([['12:30','travel_duration'],['12:00','duration']])('distinguishes rejected travel interval ending %s without logging private content',async(endTime,reason)=>{
 const result=output();Object.assign(result.events[0],{title:'PRIVATE trip',category:'travel',endTime});m.generate.mockResolvedValue({output:result});
 const response=await POST(request({travelMinutes:15}));expect(response.status).toBe(502);expect(await response.json()).toEqual({error:'unavailable'});
 expect(m.logError).toHaveBeenCalledWith('[mobile-planning] Request failed',undefined,expect.objectContaining({stage:'validation',reason,attempt:1}));
 expect(JSON.stringify(m.logError.mock.calls)).not.toContain('PRIVATE');expect(m.rpc.mock.calls.some(call=>call[0]==='planner_schedule_store_proposal')).toBe(false);
});
