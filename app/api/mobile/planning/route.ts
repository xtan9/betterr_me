...
export async function POST(request:Request){
 try{
  ...
  const result=await generateText({model:llmProvider(modelId),output:Output.object({schema:planningOutput}),providerOptions:structuredOutputProviderOptions,maxOutputTokens:4096,abortSignal:request.signal,
