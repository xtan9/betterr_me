...
export async function POST(request:Request){
 try{
  ...
  const options={model:llmProvider(modelId),output:Output.object({schema:planningOutput}),providerOptions:structuredOutputProviderOptions,maxOutputTokens:4096,abortSignal:request.signal,
   ...
   messages:[{role:'user' as const,content:JSON.stringify({date:input.date,timezone:input.timezone,commitments:input.commitments,needs:input.needs,goals:input.goals,travelMinutes:input.travelMinutes})}],
  };
