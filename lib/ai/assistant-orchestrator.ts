...
export const assistantOutput=z.object({
 ...
 planning:z.object({
  ...
  travelMinutes:z.number().int().min(1).max(1440).nullable().optional(),
 }).strict().nullable(),
...
For planning.travelMinutes, use an integer only for a travel duration the user explicitly confirmed. Omit it to preserve an unchanged previously confirmed duration. Return null when the user retracts it, starts a plan where it is unknown, or describes different trip durations that cannot share one value. Never derive travel duration from school arrival or departure times. Unknown travel remains flexible; do not invent a value.
