...
export async function runMcpAccessGrantCompatibility(
  ...
  try {
    ...
    const tokenRequest = fetchRequests.find(
    ...
    const tokenValidation = await validateProviderToken(tokens, metadata, clientInfo, target);
    ...
    try {
      ...
      if (!tool) {
        addGate(gates, "authenticated-mcp-operation", "fail", "Authenticated MCP session returned no callable tools.");
      } else {
        ...
        addGate(
          ...
          result.isError
            ...
            : `Official MCP client completed listTools and callTool(${tool.name}) through the delegated access grant.`,
