...
test("completes the quick interview, edits take-home pay, and previews What-if without changing the baseline", async ({ page }, testInfo) => {
  ...
  await page.getByRole("spinbutton", { name: "Income amount" }).fill("120000");
  ...
  await page.getByRole("spinbutton", { name: "Actual monthly take-home pay" }).fill("5000");
  await page.getByRole("button", { name: "Continue" }).click();

  await page.getByRole("button", { name: "Skip for now" }).click();
  await page.getByRole("spinbutton", { name: "Cash available now" }).fill("30000");
  await page.getByRole("button", { name: "Continue" }).click();
  await page.getByRole("button", { name: "Skip for now" }).click();
  await page.getByRole("button", { name: "Skip for now" }).click();
  ...
  await page.getByRole("spinbutton", { name: "Current total monthly spending" }).fill("6000");
  await page.getByRole("spinbutton", { name: "Lowest realistic monthly spending" }).fill("6000");
  await page.getByRole("button", { name: "Continue" }).click();
  await page.getByRole("button", { name: "Skip for now" }).click();
  ...
  await page.getByRole("spinbutton", { name: "Add accessible cash" }).fill("6000");
...
test("uses guided income, asset, housing, and transportation cards", async ({ page }) => {
  ...
  await rental.getByRole("spinbutton", { name: "Net rental income" }).fill("500");
  await expect(page.getByText("Dependable income total: $500/month")).toBeVisible();
  await page.getByRole("button", { name: "Continue" }).click();

  await page.getByRole("spinbutton", { name: "Cash available now" }).fill("0");
  await page.getByRole("button", { name: "Continue" }).click();
  await page.getByRole("button", { name: "Skip for now" }).click();

  const accessible = page.getByRole("group", { name: "Easy-to-withdraw investments" });
  await accessible.getByRole("button", { name: "Yes" }).click();
  await accessible.getByRole("spinbutton", { name: "Easy-to-withdraw investments" }).fill("6000");
  ...
  await deferred.getByRole("spinbutton", { name: "Tax-deferred or taxable-on-withdrawal retirement" }).fill("100000");
  await page.getByRole("button", { name: "Continue" }).click();
  ...
  await page.getByRole("spinbutton", { name: "Mortgage" }).fill("3000");
  await page.getByRole("spinbutton", { name: "Property tax" }).fill("12000");
  ...
  await page.getByRole("spinbutton", { name: "Vehicle payment or lease" }).fill("500");
  await page.getByRole("spinbutton", { name: "Car insurance" }).fill("1200");
  await page.getByRole("spinbutton", { name: "Fuel or charging" }).fill("200");
  ...
  await expect(page.getByText("$6,000", { exact: true })).toHaveCount(2);
