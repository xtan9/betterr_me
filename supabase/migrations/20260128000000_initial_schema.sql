...
-- 6. JOURNAL_ENTRIES (Main journaling table)
CREATE TABLE journal_entries (
  ...
  -- Metadata (will be updated by triggers)
  word_count INTEGER DEFAULT 0,
  
  -- Full-text search (will be updated by triggers)
...
-- 7. JOURNAL_TEMPLATES (Optional prompts/templates)
...
-- 8. HABIT_JOURNAL_LINKS (Connect habits to journal entries)
...
-- 9. JOURNAL_MEDIA (Photos, voice notes, etc.)
...
-- 10. USER_STATS (Enhanced with journal metrics)
...
-- 13. DAILY_SUMMARIES (Daily progress summaries)
...
-- Function to update journal entry word count and search vector
CREATE OR REPLACE FUNCTION update_journal_entry_metadata()
RETURNS TRIGGER AS $$
BEGIN
  -- Update word count
  NEW.word_count = length(regexp_replace(NEW.content->>'body', '\s+', ' ', 'g'));
...
-- Trigger to update journal entry metadata (word_count and search_vector)
CREATE TRIGGER update_journal_entry_metadata_trigger
...
-- View for monitoring slow queries (commented out - requires pg_stat_statements access)
