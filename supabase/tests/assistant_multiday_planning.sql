...
begin
 ...
 foreach action in array array['event-edit','event-remove','all-day-remove'] loop
  if action='all-day-remove' then
   perform public.calendar_capture_command('edit',gen_random_uuid(),id,version,'{"start_time":null,"end_time":null,"end_date":"2030-01-14"}') from public.calendar_events where id=midnight_id;
   if exists(select 1 from public.calendar_events where id=midnight_id and (start_time is not null or end_time is not null or end_date<>'2030-01-14')) then raise exception 'All-day setup failed';end if;
   action:='event-remove';
  end if;
 ...
 perform public.calendar_capture_command('edit',gen_random_uuid(),id,version,'{"start_time":"23:00","end_date":"2030-01-15","end_time":"00:01"}') from public.calendar_events where id=midnight_id;
 ...
 set local role authenticated;
 perform public.calendar_capture_command('edit',gen_random_uuid(),id,version,'{"start_time":null,"end_time":null}') from public.calendar_events where id=midnight_id;
 if exists(select 1 from public.calendar_events where id=midnight_id and (start_time is not null or end_time is not null)) then raise exception 'Outside all-day setup failed';end if;
 select jsonb_build_array(jsonb_build_object('id',gen_random_uuid(),'kind','event-remove','targetId',id,'expectedVersion',version,'changes','{}'::jsonb)) into actions from public.calendar_events where id=midnight_id;
 set local role authenticated;
 body:=body||jsonb_build_object('contextVersion',public.planner_horizon_context('2030-01-01','2030-01-14')->>'version','events',actions);
 ...
 set local role authenticated;
 perform public.calendar_capture_command('remove',gen_random_uuid(),id,version) from public.calendar_events where id=midnight_id;
 ...
 set local role authenticated;
 perform public.calendar_capture_command('edit',gen_random_uuid(),id,version,'{"start_time":null,"end_time":null}') from public.calendar_events where user_id=auth.uid() and title='Removal limit';
 if exists(select 1 from public.calendar_events where title='Removal limit' and (start_time is not null or end_time is not null)) then raise exception 'Distributed all-day setup failed';end if;
 ...
 set local role authenticated;
 perform public.calendar_capture_command('edit',gen_random_uuid(),id,version,'{"start_date":"2030-01-01","end_date":"2030-01-01"}') from public.calendar_events where user_id=auth.uid() and title='Removal limit';
