...
export const OWNERSHIP_REGISTRY = [
  ...
  rule("settings", [/^components\/settings\//, /^app\/(?:dashboard\/settings|api\/profile|api\/current-profile|api\/preferences|api\/profile-details|api\/user-time-zone)\//, /^lib\/(?:current-profile\.ts|preferences|profile-preference-cache|profile-preferences|submit-profile-preference-intent|db\/profiles|hooks\/(?:use-current-profile|use-profile-preferences|use-profile-theme|use-timezone)|validations\/(?:preferences|profile))/,], fullE2E()),
