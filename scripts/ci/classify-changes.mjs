...
export const OWNERSHIP_REGISTRY = [
  ...
  rule("settings", [/^components\/settings\//, /^app\/(?:dashboard\/settings|api\/profile|api\/current-profile|api\/preferences|api\/profile-details|api\/user-time-zone)\//, /^lib\/legacy-telemetry\.ts$/, /^lib\/(?:current-profile\.ts|preferences|profile-preference-cache|profile-preferences|submit-profile-preference-intent|db\/(?:notifications|profiles)|hooks\/(?:use-appearance|use-current-profile|use-localization|use-notifications|use-profile-preferences|use-profile-theme|use-timezone)|validations\/(?:preferences|profile))/,], fullE2E()),
  rule("cron", [/^app\/api\/(?:cron|email|push|reminder-defaults|reminders)\//, /^hooks\/use-push-notifications\./, /^lib\/(?:reminders|push|email)\//, /^lib\/db\/notifications\./, /^lib\/(?:db|validations)\/(?:push-subscriptions|reminder-defaults|reminders)\./], fullE2E()),
