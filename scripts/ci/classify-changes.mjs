...
export const OWNERSHIP_REGISTRY = [
  ...
  rule("chat", [/^(?:app|components)\/chat\//, /^app\/api\/(?:chat|conversations|mobile)\//, /^lib\/(?:ai|chat|db\/(?:chat|conversation)|validations\/chat)/], fullE2E()),
  ...
  rule("authentication", [/^app\/(?:auth|api\/auth|\.well-known)\//, /^components\/auth(?:\/|-)/, /^lib\/(?:auth|supabase|google)\//], fullE2E()),
