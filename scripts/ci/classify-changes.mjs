...
export const OWNERSHIP_REGISTRY = [
  ...
  rule("settings", [/^components\/settings\//, /^app\/(?:dashboard\/settings|api\/profile)\//, /^lib\/(?:preferences|profile-preference-cache|profile-preferences|submit-profile-preference-intent|db\/profiles|hooks\/(?:use-profile-theme|use-timezone)|validations\/(?:preferences|profile))/,], fullE2E()),
