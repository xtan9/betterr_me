...
export const OWNERSHIP_REGISTRY = [
  ...
  rule("workouts", [/^app\/(?:api\/)?(?:workouts|exercises|routines)\//, /^app\/api\/admin\/sync-exercise-media\//, /^components\/fitness\//, /^lib\/(?:db\/(?:exercise|routine|workout)|exercisedb\/|fitness\/|hooks\/use-(?:active-workout|exercise|routine|workout)|validations\/(?:exercise|exercise-media|routine|workout))/,], fullE2E()),
  ...
  rule("settings", [/^components\/settings\//, /^app\/(?:dashboard\/settings|api\/profile)\//, /^lib\/(?:preferences|db\/profiles|hooks\/use-timezone|validations\/(?:preferences|profile))/,], fullE2E()),
  rule("cron", [/^app\/api\/(?:cron|email|push|reminder-defaults|reminders)\//, /^hooks\/use-push-notifications\./, /^lib\/(?:reminders|push|email)\//, /^lib\/(?:db|validations)\/(?:push-subscriptions|reminder-defaults|reminders)\./], fullE2E()),
  rule("control-plane", [/^(?:app|components)\/control-plane\//, /^app\/api\/control-plane\//, /^components\/admin\//, /^lib\/control-plane\//], fullE2E({ fullTests: true })),
  ...
  rule("configuration", [/^(?:package\.json|pnpm-lock\.yaml|tsconfig\.json|vitest\.config\.ts|eslint\.config\.mjs|next\.config\.ts|playwright\.config\.ts|proxy\.ts|tailwind\.config\.ts|postcss\.config\.mjs|lighthouserc\.js|vercel\.json|stryker\.config\.mjs|components\.json|\.env\.example)$/], { ...fullE2E({ fullTests: true, fullLint: true }), performance: true }),
...
/**
 * @param {{ eventName?: string, baseSha?: string, headSha?: string, fallbackReason?: string }} comparison
 */
export function classifyComparison({ eventName = "pull_request", baseSha, headSha, fallbackReason } = {}) {
