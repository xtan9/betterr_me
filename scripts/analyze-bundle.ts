 ...
 *
 * Targets:
 * - Total JS < 1500KB gzipped (increased from 850KB to accommodate money feature)
...
function analyze() {
  ...
  console.log(`  Total JS target: < 1500KB gzipped`);
  console.log(`  Per-chunk target: < 130KB gzipped`);

  // Check thresholds
  let hasIssues = false;

  if (totalGzip > 1500 * 1024) {
    console.log(`\n  ⚠️  Total JS gzipped (${formatBytes(totalGzip)}) exceeds 1500KB target`);
    hasIssues = true;
  } else {
    console.log(`\n  ✅ Total JS gzipped within 1500KB target`);
  }
  ...
  writeFileSync(
    ...
    JSON.stringify({
      ...
      totalGzip,
      oversizedChunks,
      thresholds: { totalGzip: 1500 * 1024, perChunkGzip: 130 * 1024 },
      passed: !hasIssues,
    }, null, 2)
  );
