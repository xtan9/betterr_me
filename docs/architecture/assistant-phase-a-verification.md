...
The full deployed synthetic probe passed on backend `baef83a6`: golden discovery (21.6 seconds), skip draft with explicit assumptions (44.0 seconds), all three durable preferences recalled in a new conversation, a temporary one-month correction with unchanged durable baseline, later four-day recall, two-owner isolation, and zero task/calendar mutations. Both probe accounts were cleaned up. Browser verification is not physical-device verification.

The existing manual one-day planner independently rejected generated outputs at `events.*.startTime` with `invalid_string`. Its unchanged HH:MM validator now has an explicit schema description and generation instruction; one schema retry regenerates from original context before proposal storage. Regression cases reject seconds/ISO timestamps, recover through valid regenerated output, exhaust safely, and preserve no-retry behavior for protected-event overlap. Next Action's generated explanation and mobile English/Chinese guidance use ordinary preview/accept language instead of internal capture terminology. Live guided-planner Undo and physical-device verification remain pending.
