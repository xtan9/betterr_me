...
Backend PR #1027 added allowlisted schema issue paths/codes. The live skip failure was identified as `too_big` at `planning.draft`, not truncated generation or an authentication failure. The 4,000-character validator remains unchanged. Generation instructions now budget a concise prose outline, with one regeneration per turn for this exact validation failure, only before any streamed text is published. Invalid output is never truncated, persisted, or included in the retry context. Failed retries still fail closed. Route regression coverage includes JSON/stream recovery, retry exhaustion with no writes, no retry after publication, and one shared retry budget across initial/calendar-aware generation. Full deployed skip/memory verification remains pending until this fix is deployed and the probe completes.
