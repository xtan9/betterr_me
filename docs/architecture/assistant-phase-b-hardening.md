...
- An additional production DST-spanning preview asked for exact times already present in the conversation draft. The saved-session planning route supplied only summarized facts, so details omitted from extraction were unavailable to preview generation. It now also reads up to 40 recent messages from the authenticated owner's same conversation, bounded by the saved session's update time, and supplies only chronological user text as untrusted source data. Assistant-generated prose and later messages are excluded. Session-version checks, exact preview validation, storage conflicts and acceptance remain unchanged. Regression cases verify two authenticated-owner filters, conversation/time bounds, source ordering, assistant-text exclusion, fail-closed history reads, and sanitized provider failures. Production re-verification of the DST scenario is pending.
