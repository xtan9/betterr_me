...
| --- | --- |
| HTTP creation/query/Series mutation | `app/api/recurring-tasks/route.ts` and `[id]/route.ts` use the authenticated capabilities; legacy fields are translated at the compatibility edge and route tests cover operation IDs, opaque versions, effective dates, and typed failures. |
| AI creation/query/Series mutation | `lib/ai/tools/tasks.ts` uses the same public Series command/query capabilities and response adapter; recurring-task tests and `series-state-parity.test.ts` prove canonical HTTP/AI inputs. |
...
| Task writes | `lib/tasks/writes.ts`, `occurrence-adapter.ts`, and the private `series-state-adapter.ts` remain compatibility seams for task-scoped commands; supported Series-definition and Series-state routes do not construct them. |
