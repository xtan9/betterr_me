...
Each selected layer is atomic. The authenticated Calendar query ensures Coverage Horizon before reading materialized Task Occurrences; habit acquisition requires both habits and completion logs. Selected layers start concurrently, and successful items have deterministic display ordering. Capability-shaped acquisition ports hide Supabase, with one Supabase composition adapter supplying the concrete reads. The Calendar query carries the shared structured Coverage completeness fact so an incomplete task layer cannot be classified as complete.
