...
must_haves:
  truths:
    ...
    - "Deleting a project nullifies project_id on associated tasks (ON DELETE SET NULL)"
    - "Archiving a project changes its status to 'archived' without affecting tasks"
  artifacts:
...
<task type="auto">
  ...
  <action>
    ...
    **Task validation extension** (`lib/validations/task.ts`):
    ...
    - Add same fields to `taskUpdateSchema` if they aren't already present
  </action>
  <verify>
