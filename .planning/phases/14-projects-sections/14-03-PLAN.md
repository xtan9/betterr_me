...
<task type="auto">
  ...
  <action>
    ...
    **Task list adjustment** (`components/tasks/task-list.tsx`):
    - The existing TaskList component may need adjustment to accept a filtered tasks array rather than fetching its own. If it already accepts tasks as props, minimal changes needed. If it has its own SWR call, refactor to accept tasks as a prop from the parent (tasks-page-content.tsx now controls data fetching and grouping).
    - Ensure TaskList can render a subset of tasks (standalone tasks within a section) without its own filtering logic.
  </action>
  <verify>
