...
3. Update the `computeMissedDays` catch block (currently line ~107-110):
   ...
   - Keep `return { ...habit, missed_scheduled_periods: 0, previous_streak: 0 };` -- zeros are the mathematical identity and there is no cached prior value available. Document this with a comment: `// Zeros as fallback: no prior cached value available (see RESEARCH.md Pitfall 5)`.
