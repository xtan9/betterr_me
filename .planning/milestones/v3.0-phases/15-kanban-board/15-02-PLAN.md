...
must_haves:
  truths:
    ...
    - "Dragging a card to a different column changes its status, and the change persists after page reload"
...
Purpose: Deliver the primary kanban interaction — users see their project's tasks organized by status and can drag cards between columns to change status with instant visual feedback and persistent state.
