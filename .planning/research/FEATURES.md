...
| T6 | Fix `computeMissedDays` silent error swallowing | Fragility | Dashboard route catches all errors in `computeMissedDays` and silently returns `missed_scheduled_days: 0`. A bug in absence computation produces silently incorrect data with no observable failure. At minimum, errors must be logged. | LOW | `app/api/dashboard/route.ts:107-110` |
